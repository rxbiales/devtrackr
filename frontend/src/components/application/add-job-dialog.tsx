...
import { Plus, Briefcase, Building2, Globe, Monitor, CalendarIcon, FileText, UserCog } from "lucide-react"
...
const formSchema = z.object({
  ...
  role: z.string().min(2, "Descreva o nível ou função (ex: Júnior, Especialista)"), // 👈 ADICIONADO
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 pt-4">
            <div className="grid grid-cols-1 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2"><Briefcase className="h-3.5 w-3.5 text-muted-foreground" /> Título da Vaga</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2"><UserCog className="h-3.5 w-3.5 text-muted-foreground" /> Nível / Role</FormLabel>
                    <FormControl><Input placeholder="Ex: Pleno, Tech Lead, Backend" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger><SelectValue placeholder="Selecione um currículo" /></SelectTrigger>
                    </FormControl>
                    <SelectContent>
